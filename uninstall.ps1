...
# install.ps1의 같은 이름 함수와 반드시 같은 규칙이어야 한다 — 확장 후 정규화해서 비교.
# User PATH는 REG_EXPAND_SZ라 %LOCALAPPDATA%\... 같은 압축형이 섞인다. 문자열 그대로
# 비교하면 우리가 심은 항목을 못 찾아 제거하지 못한다.
...
function Test-ArtifactAllowed([string]$Path) {
    ...
    $exact = @(
        ...
        # statusline.sh는 settings.json의 statusLine이 가리키는 wrapper, claude-hud.json은
        # 그 wrapper가 띄우는 플러그인의 설정 override다. 플러그인 바이너리와 마켓플레이스
        # 등록은 claude plugin CLI 소유라 여기 없다 — 설정만 소유한다.
        (Join-Path $homeRoot '.claude\statusline.sh'), (Join-Path $homeRoot '.claude\claude-hud.json'),
...
function Remove-ManagedValue([string]$Key) {
    ...
    if ($Key.StartsWith('git:')) {
        ...
        $segment = $Key.Substring(9); $current = Get-DotfilesUserEnvironment 'PATH'; if($null -eq $current){$current=''}; $parts = @($current.Split(';', [StringSplitOptions]::None)); $matches = @($parts | Where-Object { Test-UserPathSegmentMatch $_ $segment }); $count = $matches.Count
        if ($entry.pending) {
        ...
        if ($count -eq 1 -and $entry.installed -eq 'present') { $removed=$false; $new=@($parts | Where-Object { if (-not $removed -and (Test-UserPathSegmentMatch $_ $segment)) { $removed=$true; $false } else { $true } }); Set-DotfilesUserEnvironment 'PATH' ($new -join ';') }
        elseif ($count -eq 0 -and -not $entry.before.present) { }
        else { Write-Preserve "Ambiguous PATH segment preserved: $segment"; return $false }
        $check=Get-DotfilesUserEnvironment 'PATH';if($null -eq $check){$check=''};$checkCount=@($check.Split(';',[StringSplitOptions]::None)|Where-Object{Test-UserPathSegmentMatch $_ $segment}).Count
