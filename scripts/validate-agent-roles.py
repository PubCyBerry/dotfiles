...
사용: uv run --with pyyaml --python 3.11 scripts/validate-agent-roles.py [roles_dir]
"""
...
def check_role(d: Path) -> None:
    ...
    claude_text = (d / "claude.frontmatter").read_text(encoding="utf-8") + body
    for error in validate_text(claude_text, role):
        fail(role, f"claude: {error}")
    ...
    if effort is not None and (
        not isinstance(effort, str) or effort not in CODEX_EFFORTS
    ):
    ...
    if sandbox is not None and (
        not isinstance(sandbox, str) or sandbox not in CODEX_SANDBOX
    ):
