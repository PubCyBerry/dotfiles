...
#   Merge    : $SourcePath가 이미 $DestPath의 현재 내용을 흡수한 병합 산출물이다.
#              앱이 자기 설정 파일을 스스로 고치는 경우(Claude Code의 settings.json,
#              Codex의 config.toml, Antigravity의 hooks.json)가 정상이라, 해시 대조로
#              막으면 그 파일에는 다시는 관리 값이 들어가지 않는다. 병합은 destination
#              우선이라 덮어써도 사용자 값이 사라지지 않으므로 그 게이트만 건너뛴다.
function Install-ManagedFile([string]$SourcePath, [string]$DestPath, [ValidateSet('Takeover','Skip','Merge')] [string]$Collision = 'Takeover') {
    ...
    if ($entry) {
        ...
        if ($exists -ne $expectedExists -or ($exists -and $beforeHash -ne $expectedHash)) {
            if ($Collision -ne 'Merge') {
                Write-Warning "Managed file changed or missing; preserving: $DestPath"
                return $false
            }
            # 병합 산출물은 현재 내용을 이미 담고 있으므로 소유권 기준만 지금 상태로 다시 잡는다.
            # before 스냅샷은 uninstall의 복원 대상이라 건드리지 않는다.
            $entry.pending = $false
            foreach ($field in @('targetHash','previousHash','previousExists')) { $null = $entry.Remove($field) }
            $entry.installedHash = $beforeHash
        }
...
function Begin-ManagedPackage([string]$Name, [bool]$BeforePresent, [string]$BeforeValue, [string]$Prefix = '') {
    ...
    if ($Prefix -and $existing -and ((-not $existing.prefix) -or $existing.prefix -cne $Prefix)) {
        ...
        #   2) Node LTS 업그레이드 — prefix가 node-versions/<version>/installation 이라
        #      버전이 오르면 값이 바뀐다. 이 스크립트가 매번 LTS를 설치하므로 정상적인 일이다.
        # 새 prefix가 안정 경로이기만 하면 두 경우 모두 교정하고 진행한다. 여기서 막으면
        # Node를 한 번 올린 뒤부터 그 패키지가 영영 설치되지 않는다.
        if (Test-EphemeralNpmPrefix $Prefix) {
            Write-Warning "npm prefix is not a stable path; preserving package ownership: $Name"
            return $false
        }
        Write-Warning "npm prefix changed in receipt; re-baselining ownership: $Name"
...
# PATH 항목 하나가 특정 디렉터리를 가리키는지 판정한다.
#
# 레지스트리의 User PATH는 REG_EXPAND_SZ라 항목이 %LOCALAPPDATA%\... 처럼 접두사로
# 압축돼 있을 수 있다(scripts\clean-env.ps1이 실제로 그렇게 쓴다). 문자열을 그대로
# 비교하면 같은 디렉터리를 "없음"으로 보고, 관리 항목을 잃어버렸다고 판단한다.
# uninstall.ps1의 Test-UserPathSegmentMatch와 반드시 같은 규칙이어야 한다.
function Test-UserPathSegmentMatch([string]$Segment, [string]$Dir) {
    if ([string]::IsNullOrWhiteSpace($Segment) -or [string]::IsNullOrWhiteSpace($Dir)) { return $false }
    try {
        $a = [IO.Path]::GetFullPath([Environment]::ExpandEnvironmentVariables($Segment.Trim())).TrimEnd('\')
        $b = [IO.Path]::GetFullPath([Environment]::ExpandEnvironmentVariables($Dir.Trim())).TrimEnd('\')
    } catch { return $false }
...
function Add-ToUserPath([string]$Dir) {
    ...
    $present = @($userPath -split ';' | Where-Object { Test-UserPathSegmentMatch $_ $Dir }).Count -gt 0
...
function Merge-CodexConfig([string]$SourcePath, [string]$DestPath) {
    ...
    try {
        ...
        } elseif (Install-ManagedFile $tmp $DestPath Merge) {
            Write-Host "    Merged config.toml (existing values preserved)"
        }
    } finally {
        Remove-Item $tmp -Force -ErrorAction SilentlyContinue
...
function ConvertTo-CanonicalJson([string]$Text) {
    ...
    # jq 부재를 여기서 흡수한다. $ErrorActionPreference = 'Stop' 아래에서 없는 native
    # command 호출은 terminating error라, 가드 없이 두면 Add-InstallFailure 계약을
    # 우회해 install 전체가 끊긴다 — 실제로 겪은 회귀다.
...
function Install-Rhwp {
    ...
    $ok = $true
    # 도구 부재는 config.toml 병합과 같은 정책으로 건너뛴다 — 설치 전체를 실패시키지 않는다.
    #
    # jq는 호스트를 가리지 않고 필요하다. 비교를 언제나 정규화된 JSON으로 하기 때문에
    # (ConvertTo-CanonicalJson) Codex의 TOML entry도 그 경로를 지난다. 그래서 jq 가드가
    # Codex 분기까지 감싸야 한다 — Claude/Gemini만 감싸면 yq는 있고 jq만 없는 머신에서
    # Codex 등록이 실패로 기록돼 install이 실패로 끝난다.
    $hasJq = [bool](Get-Command jq -ErrorAction SilentlyContinue)
    if (-not $hasJq) {
        Write-Host '    [!] jq not found; skipping MCP registration.'
    } else {
        # Codex는 TOML 편집이라 yq가 하나 더 필요하다.
...
function Deploy-HerdrConfig {
    ...
    try {
        ...
        } elseif (Install-ManagedFile $staged $dst Merge) {
...
function Update-FnmStatusLine([string]$SettingsPath, [string]$FnmRoot, [string]$NodeVersion) {
    ...
    try {
        ...
        try {
            ...
            # 산출물은 현재 settings.json을 읽어 statusLine 한 키만 바꾼 것이므로 병합
            # 산출물이다. Claude Code가 자기 설정을 다시 쓰는 파일이라 Takeover면 그
            # 다음부터 영구히 보존으로 막힌다 — Merge와 같은 근거다.
            if (Install-ManagedFile $tmp $SettingsPath Merge) {
                Write-Host "    Patched statusLine node path: $($match.Value) -> $nodeExe"
                return $true
            }
            return $false
...
# statusline.sh: settings.json의 statusLine이 가리키는 wrapper. claude-hud 출력과 ccusage
# 출력을 합친다. Takeover 근거는 hook 스크립트와 같다 — 이름으로 소유하는 파일이라 Skip이면
# 구버전 사본이 "사용자 파일"로 판정돼 영구히 남는다.
$statuslineSrc = Join-Path $ROOT "config\claude\statusline.sh"
if (Test-Path $statuslineSrc) {
    if (Install-ManagedFile $statuslineSrc (Join-Path $ClaudeDir "statusline.sh") Takeover) {
        Write-Host "    Copied statusline.sh"
    }
} else {
    Write-Host "    [!] config\claude\statusline.sh not found, skipping."
}

# claude-hud.json: 플러그인 설정 override. 플러그인 디렉터리 밖(~\.claude\claude-hud.json)이라
# 플러그인 업데이트가 지우지 않고, claude-hud 자신도 이 경로에는 쓰지 않는다
...
$hudConfigSrc = Join-Path $ROOT "config\claude\claude-hud.json"
if (Test-Path $hudConfigSrc) {
    if (Install-ManagedFile $hudConfigSrc (Join-Path $ClaudeDir "claude-hud.json") Takeover) {
        Write-Host "    Copied claude-hud.json"
    }
} else {
    Write-Host "    [!] config\claude\claude-hud.json not found, skipping."
}
