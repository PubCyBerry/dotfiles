...
- herdr는 설정만 제거 대상이다. `%APPDATA%\herdr\config.toml`(Windows) / `~/.config/herdr/config.toml`(Linux·macOS)이 설치 당시 내용과 정확히 같을 때만 제거·복원한다. 바이너리는 공식 installer(macOS는 Homebrew)가 설치하고 herdr가 스스로 업데이트하므로 install이 애초에 소유하지 않는다 — uninstall도 손대지 않는다. herdr 자체를 지우려면 그 설치 경로의 언인스톨 수단을 쓴다.
