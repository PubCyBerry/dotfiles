...
자동 제거하지 않는 범위: receipt에 없는 Node 버전, skills(`npx skills`가 설치하므로 `npx skills remove`로 지운다), plugins/marketplaces, macOS defaults, legacy Codex skills, 구 로컬 skill 배포분(`~/.gemini/config/skills/`), cache와 사용자 data.
