...
# install.sh는 Linux/macOS 진입점이다. Git Bash(MSYS)에서 돌리면 네이티브 jq.exe에
# 넘기는 POSIX 경로가 Windows 경로로 자동 변환돼(`/tmp/...` -> `C:/Users/.../Temp/...`)
# receipt에 셸 변수와 다른 문자열이 들어간다. 그 차이는 제품 동작이 아니라 실행 환경의
# 문제라 여기서 검증할 것이 없다. Windows의 같은 계약은 install-receipt.ps1이 본다.
case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
        echo 'Unix install receipt: SKIP (Git Bash — install.sh targets Linux/macOS)'
        exit 0
        ;;
esac
...
upgraded_prefix="$HOME/.local/share/fnm/node-versions/v2.0.0/installation"
mkdir -p "$upgraded_prefix"
begin_managed_package npm:test-prefix true 1 "$upgraded_prefix" || fail 'stable npm prefix change blocked the package'
[[ "$(jq -r '.packages["npm:test-prefix"].prefix' "$RECEIPT_PATH")" == "$upgraded_prefix" ]] || fail 'stable npm prefix not re-baselined'
cancel_managed_package npm:test-prefix
receipt_commit --arg prefix "$stable_prefix" '.packages["npm:test-prefix"].prefix=$prefix'
...
grep -Fq 'user-hud' "$settings" || fail 'fnm user modification not preserved'
grep -Fq 'v23.0.0' "$settings" || fail 'fnm did not patch the node path'
...
merge_dst="$TMP/merge/registry.json"; merge_src="$TMP/merge/merged.json"
mkdir -p "$(dirname "$merge_dst")"
printf '{"user":1}' > "$merge_dst"
printf '{"user":1,"managed":1}' > "$merge_src"
install_managed_file "$merge_src" "$merge_dst" merge || fail 'merge install'
merge_before="$(jq -r --arg path "$merge_dst" '.artifacts[$path].before.hash' "$RECEIPT_PATH")"

printf '{"user":1,"managed":1,"app":true}' > "$merge_dst"
! install_managed_file "$merge_src" "$merge_dst" takeover || fail 'takeover overwrote an app-modified file'
printf '{"user":1,"managed":2,"app":true}' > "$merge_src"
install_managed_file "$merge_src" "$merge_dst" merge || fail 'merge refused an app-modified file'
[[ "$(cat "$merge_dst")" == '{"user":1,"managed":2,"app":true}' ]] || fail 'merge content mismatch'
[[ "$(jq -r --arg path "$merge_dst" '.artifacts[$path].before.hash' "$RECEIPT_PATH")" == "$merge_before" ]] || fail 'merge clobbered the before snapshot'
[[ "$(jq -r --arg path "$merge_dst" '.artifacts[$path].installedHash' "$RECEIPT_PATH")" == "$(file_hash "$merge_dst")" ]] || fail 'merge did not record the installed hash'
