...
try {
    ...
    # Node LTS 업그레이드는 prefix를 <fnm_root>/node-versions/<version>/installation에서 바꾼다.
    # 안정 경로끼리의 변경은 소유권을 이어받아야 한다 — 여기서 막으면 Node를 한 번 올린
    # 뒤부터 그 패키지가 영영 설치되지 않는다(실제로 겪은 회귀).
    $upgradedPrefix = Join-Path $env:APPDATA 'fnm\node-versions\v2.0.0\installation'
    Assert (Begin-ManagedPackage 'npm:test-prefix' $true '1' $upgradedPrefix) 'stable npm prefix change blocked the package'
    Assert ($script:Receipt.packages['npm:test-prefix'].prefix -eq $upgradedPrefix) 'stable npm prefix was not re-baselined'
    Cancel-ManagedPackage 'npm:test-prefix'
    $script:Receipt.packages['npm:test-prefix'].prefix = $stablePrefix
    ...
    # 산출물이 현재 settings.json에서 파생되므로 사용자 값은 그대로 살아남고, 관리 값
    # (node 경로)은 계속 갱신된다. Takeover였을 때는 여기서 막혀 다시는 갱신되지 않았다.
    Assert (Update-FnmStatusLine $settings $fnmRoot 'v23.0.0') 'fnm refused an app-modified settings.json'
    Assert ((Get-Content $settings -Raw) -match 'user-hud') 'fnm user modification not preserved'
    Assert ((Get-Content $settings -Raw) -match 'v23\.0\.0') 'fnm did not patch the node path'

    # Claude Code / Codex / Antigravity는 자기 설정 파일을 스스로 다시 쓴다. 그 파일을
    # Takeover로 다루면 다음 실행이 "사용자가 고쳤다"로 보고 영구히 보존해, 관리 값이
    # 다시는 들어가지 않는다(실제로 겪은 회귀). 병합 산출물은 현재 내용을 이미 담고
    # 있으므로 Merge는 그 게이트만 건너뛴다.
    $mergeDst = Join-Path $temp 'merge\registry.json'
    $mergeSrc = Join-Path $temp 'merge\merged.json'
    New-Item -ItemType Directory -Force -Path (Split-Path $mergeDst) | Out-Null
    Set-Content $mergeDst '{"user":1}' -NoNewline
    Set-Content $mergeSrc '{"user":1,"managed":1}' -NoNewline
    Assert (Install-ManagedFile $mergeSrc $mergeDst Merge) 'merge install failed'
    $mergeKey = [IO.Path]::GetFullPath($mergeDst)
    $mergeBefore = $script:Receipt.artifacts[$mergeKey].before.hash

    Set-Content $mergeDst '{"user":1,"managed":1,"app":true}' -NoNewline
    Assert (-not (Install-ManagedFile $mergeSrc $mergeDst Takeover)) 'takeover overwrote an app-modified file'
    Set-Content $mergeSrc '{"user":1,"managed":2,"app":true}' -NoNewline
    Assert (Install-ManagedFile $mergeSrc $mergeDst Merge) 'merge refused an app-modified file'
    Assert ((Get-Content $mergeDst -Raw) -eq '{"user":1,"managed":2,"app":true}') 'merge content mismatch'
    Assert ($script:Receipt.artifacts[$mergeKey].before.hash -eq $mergeBefore) 'merge clobbered the before snapshot'
    Assert ($script:Receipt.artifacts[$mergeKey].installedHash -eq (Get-FileHash $mergeDst -Algorithm SHA256).Hash.ToLowerInvariant()) 'merge did not record the installed hash'

    # User PATH는 REG_EXPAND_SZ라 항목이 %LOCALAPPDATA%\... 로 압축돼 있을 수 있다.
    # 문자열 그대로 비교하면 우리가 심은 항목을 "지워졌다"로 오판한다.
    Assert (Test-UserPathSegmentMatch '%LOCALAPPDATA%\path-demo' (Join-Path $env:LOCALAPPDATA 'path-demo')) 'compressed PATH segment not matched'
    Assert (Test-UserPathSegmentMatch (Join-Path $env:LOCALAPPDATA 'path-demo\') (Join-Path $env:LOCALAPPDATA 'path-demo')) 'trailing separator not normalized'
    Assert (-not (Test-UserPathSegmentMatch '%LOCALAPPDATA%\path-demo' (Join-Path $env:LOCALAPPDATA 'other-demo'))) 'different directories matched'
    Assert (-not (Test-UserPathSegmentMatch '' (Join-Path $env:LOCALAPPDATA 'path-demo'))) 'empty segment matched'
