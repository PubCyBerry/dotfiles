...
'@ | Set-Content $legacyOnlyDst -Encoding utf8 -NoNewline
    ...
    # 이 저장소가 과거에 심은 statusline 명령은 managed 값(wrapper)으로 교체돼야 한다. 두 세대가
    # 있다. (1) /claude-hud:setup이 만든 명령줄 — 머신마다 경로가 달라 부분 일치로 본다.
    # (2) `ccusage statusline` — 이 저장소가 배포한 고정 문자열이라 정확히 일치할 때만 소유로
    # 인정한다. 사용자가 직접 넣은 statusLine(위 user-sentinel)은 같은 merge에서 보존된다 —
    # 셋을 구분하는 것이 이 검사다.
...
'@ | Set-Content $legacyHudDst -Encoding utf8 -NoNewline
    ...
    jq -e '.statusLine.command == "bash ~/.claude/statusline.sh" and .statusLine.padding == 0' $legacyHudDst | Out-Null
    Assert-True ($LASTEXITCODE -eq 0) "claude-hud statusline이 wrapper로 교체되지 않았습니다."

    $legacyCcusageDst = Join-Path $work "legacy-ccusage.json"
    @'
{"statusLine":{"type":"command","command":"ccusage statusline","padding":0}}
'@ | Set-Content $legacyCcusageDst -Encoding utf8 -NoNewline
    Merge-JsonRegistry $claudeSrc $legacyCcusageDst
    jq -e '.statusLine.command == "bash ~/.claude/statusline.sh"' $legacyCcusageDst | Out-Null
    Assert-True ($LASTEXITCODE -eq 0) "ccusage statusline 값이 wrapper로 교체되지 않았습니다."

    # `ccusage statusline`을 감싼 사용자 정의 명령은 소유가 아니다 — 정확 일치가 아니므로 보존한다.
    $userCcusageDst = Join-Path $work "user-ccusage.json"
    @'
{"statusLine":{"type":"command","command":"ccusage statusline --no-offline"}}
'@ | Set-Content $userCcusageDst -Encoding utf8 -NoNewline
    Merge-JsonRegistry $claudeSrc $userCcusageDst
    jq -e '.statusLine.command == "ccusage statusline --no-offline"' $userCcusageDst | Out-Null
    Assert-True ($LASTEXITCODE -eq 0) "사용자가 고친 ccusage statusline이 보존되지 않았습니다."
