...
jq -e '.statusLine.command == "bash ~/.claude/statusline.sh" and .statusLine.padding == 0' "$legacy_hud_dst" >/dev/null

legacy_ccusage_dst="$work/legacy-ccusage.json"
cat > "$legacy_ccusage_dst" <<'JSON'
{"statusLine":{"type":"command","command":"ccusage statusline","padding":0}}
JSON
merge_json_registry "$claude_src" "$legacy_ccusage_dst"
jq -e '.statusLine.command == "bash ~/.claude/statusline.sh"' "$legacy_ccusage_dst" >/dev/null

# `ccusage statusline`을 감싼 사용자 정의 명령은 소유가 아니다 — 정확 일치가 아니므로 보존한다.
user_ccusage_dst="$work/user-ccusage.json"
cat > "$user_ccusage_dst" <<'JSON'
{"statusLine":{"type":"command","command":"ccusage statusline --no-offline"}}
JSON
merge_json_registry "$claude_src" "$user_ccusage_dst"
jq -e '.statusLine.command == "ccusage statusline --no-offline"' "$user_ccusage_dst" >/dev/null
